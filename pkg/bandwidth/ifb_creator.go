...
func CreateClsactQdisc(rateInBits, burstInBits uint64, hostDeviceName string) error {
...
func CreateEgressQdisc(rateInBits, burstInBits uint64, hostDeviceName string, ifbDeviceName string) error {
	...
	if err != nil {
		return fmt.Errorf("add ingress filter: %s", err)
	}
	egressFilter := &netlink.U32{
		FilterAttrs: netlink.FilterAttrs{
			LinkIndex: hostDevice.Attrs().Index,
